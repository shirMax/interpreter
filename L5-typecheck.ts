...
import {
    ...
    isDefineTypeExp, isTypeCaseExp, DefineTypeExp, TypeCaseExp, CaseExp, makePrimOp, CExp, VarDecl
} from "./L5-ast";
...
import {
    ...
    makeSymbolTExp,
    makePairTExp,
    UserDefinedNameTExp, makeUserDefinedTExp
} from "./TExp";
...
import { Closure, isClosure, isEmptySExp, isSymbolSExp, makeClosure, SExpValue } from './L5-value';
...
const te1Record = (te1: Record, te2: TExp, p: Program): boolean => {
    if(isRecord(te2)) {
        return equals(te1, te2);
    }
    if(isUserDefinedTExp(te2)) {
        return isOk(getItemByName(te2.typeName,  getRecordParents(te1.typeName, p)))
    }
    if(isUserDefinedNameTExp(te2)) {
        let recordByName : Result<Record> = getRecordByName(te2.typeName, p);
        if(isOk(recordByName))
            return equals(recordByName.value, te1)
        let userDefined : Result<UserDefinedTExp> = getUserDefinedTypeByName(te2.typeName, p)
        if(isOk(userDefined))
            return isOk(getItemByName(te2.typeName,  getRecordParents(te1.typeName, p)))
    }
    return false;
...
const te1IsUserDefine = (te1: TExp, te2: TExp, p: Program) : boolean => {
    if(isRecord(te2)) {
        return false;
    }
    if(isUserDefinedTExp(te2)) {
        return equals(te2, te1)
    }
    if(isUserDefinedNameTExp(te2)) {
        let recordByName : Result<Record> = getRecordByName(te2.typeName, p);
        if(isOk(recordByName))
        ...
        let userDefined : Result<UserDefinedTExp> = getUserDefinedTypeByName(te2.typeName, p)
        if(isOk(userDefined))
            return equals(userDefined.value, te1)

    }
    return false;
}

const te1IsUserDefineName = (te1: UserDefinedNameTExp, te2: TExp, p: Program) : boolean => {
    let recordByName1 : Result<Record> = getRecordByName(te1.typeName, p);
    if(isOk(recordByName1))
        return te1Record(recordByName1.value, te2, p);
    let userDefined1 : Result<UserDefinedTExp> = getUserDefinedTypeByName(te1.typeName, p);
    if(isOk(userDefined1))
        return te1IsUserDefine(userDefined1.value, te2, p);
    return false;
}
...
const isSubType = (te1: TExp, te2: TExp, p: Program): boolean => {
    if(isAnyTExp(te2))
        return true;
    if(isRecord(te1))
        return te1Record(te1,te2,p)
    if(isUserDefinedTExp(te1))
        return te1IsUserDefine(te1,te2,p)
    if(isUserDefinedNameTExp(te1))
        return te1IsUserDefineName(te1,te2,p)
    return false;
}
...
export const initTEnv = (p: Program): TEnv => {
    ...
    const typeDefinitions : UserDefinedTExp[] = getTypeDefinitions(p);
    const records : Record[] = getRecords(p);
    for(let defineExp of definitions){
        vars.push(defineExp.var.var);
        texps.push(defineExp.var.texp);
    }
    for(let defineType of typeDefinitions){
        vars.push(defineType.typeName);
        texps.push(defineType);
        vars.push(defineType.typeName+"?");
    ...
    for(let record of records){
        ...
        texps.push(makeProcTExp(map((field) => field.te,record.fields),makeUserDefinedNameTExp(record.typeName)));
        vars.push(record.typeName+"?");
        texps.push(makeProcTExp([makeAnyTExp()], makeBoolTExp()));
    }
...
// vars: string[]; texps: TExp[]; tenv: TEnv;
...
const checkUserDefinedTypes = (p: Program): Result<true> =>
    bind(checkDuplicateUserDefine(p), () =>
        bind(checkDuplicateRecord(p) , () =>
            mapTrueResults(mapResult((userD:UserDefinedTExp) =>
                    checkRecursiveRecords(userD.records, userD)
                ,getTypeDefinitions(p)))))

const mapTrueResults = (r: Result<true[]>): Result<true> =>
    isOk(r) && r.value.length>0 ? makeOk(r.value[0]) : makeFailure("fail");


const checkDuplicateUserDefine = (p:Program) : Result<true> => {
    const usersDefine1 : UserDefinedTExp[] = getTypeDefinitions(p);
    const usersDefine2 : UserDefinedTExp[] = [];
    for(let userDefine1 of usersDefine1) {
        for(let userDefine2 of usersDefine2){
            if(equals(usersDefine1,usersDefine2))
                return makeFailure("the same type name is defined twice with different definitions");
        }
        usersDefine2.push(userDefine1);
    }
    return makeOk(true);
}

const checkDuplicateRecord = (p:Program) : Result<true> => {
    const records : Record[] = getRecords(p);
    for(let r1 of records){
        for(let r2 of records) {
            if (equals(r1.typeName, r2.typeName))
                if(isFailure(checkEqualRecords(r1,r2)))
                    return makeFailure("Fail");
        }
    }
    return makeOk(true);
}
const checkEqualRecords = (r1 : Record, r2 : Record) : Result<true> => {
    if(r1.fields.length != r2.fields.length) return makeFailure("fail");
    for(let i = 0 ; i < r1.fields.length; i++){
        if(r1.fields[i].fieldName != r2.fields[i].fieldName || r1.fields[i].te != r2.fields[i].te)
            return makeFailure("fail");
    }
    return makeOk(true);
}

const checkRecursiveRecord = (r:Record, userDefine : UserDefinedTExp) : Result<true> => {
    for(let f of r.fields)
        if(isUserDefinedNameTExp(f.te) && f.te.typeName === userDefine.typeName)
            return checkIfRecordEmpty(userDefine);
    return makeOk(true);
}

const checkRecursiveRecords = (records:Record[], userDefine : UserDefinedTExp) : Result<true> => {
    for(let r of records)
        if(isFailure(checkRecursiveRecord(r, userDefine)))
            return makeFailure("fail");
    return makeOk(true);
}

const checkIfRecordEmpty = (userDefine : UserDefinedTExp) : Result<true> => {
    for(let r of userDefine.records)
...
const checkTypeCase = (tc: TypeCaseExp, p: Program): Result<true> => {
    const udTExp : Result<UDTExp> = getTypeByName(tc.typeName, p);
    if(isOk(udTExp)) {
        if(isRecord(udTExp.value))
            return checkParents(tc, p);
        if(isUserDefinedTExp(udTExp.value))
            return isUserDefine(udTExp.value, tc);
    }
    return makeFailure("fail");
}
const checkParents = (tc: TypeCaseExp, p: Program) : Result<true> => {
    const parents : UserDefinedTExp[] = getRecordParents(tc.typeName, p)
    for(let userD of parents)
        if(isOk(isUserDefine(userD, tc)))
            return makeOk(true);
    return makeFailure("fail")
}

const isUserDefine = (userD : UserDefinedTExp, tc : TypeCaseExp) : Result<true> => {
    if(tc.cases.length != userD.records.length)
        return makeFailure("fail");
    const records : Record[] = userD.records;
    for(let record of records){
        let res : boolean = false;
        for(let c of tc.cases) {
            if (record.typeName === c.typeName && c.varDecls.length == record.fields.length)
                res = true;
        }
        if(!res)
            return makeFailure("fail");
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv, p: Program): Result<TExp> => {
    ...
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
        bind(altTE, (altTE: TExp) =>
            isOk(checkEqualType(thenTE, altTE, ifExp, p)) ? checkEqualType(thenTE, altTE, ifExp, p) : checkEqualType(altTE, thenTE, ifExp, p)));
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv, p: Program): Result<VoidTExp> => {
    ...
    const areEqualTypes = bind(constraint, (val) => checkEqualType(val,texp,exp,p))
    return bind(areEqualTypes,() => mapv(constraint, (_) => makeVoidTExp()));
};
...
export const typeofSet = (exp: SetExp, _tenv: TEnv, _p: Program): Result<TExp> =>
    bind(typeofExp(exp.var, _tenv, _p), (type1) =>
        bind(typeofExp(exp.val, _tenv, _p), (type2) => isOk(checkEqualType(type2, type1, exp, _p)) ? makeOk(makeVoidTExp()) : makeFailure("fail")))
                    ...
                    isEmptySExp(exp.val) ? makeOk(makeSymbolTExp()) :
                        isCompoundSexp(exp.val) ? makeOk(makePairTExp()) :
                            isPrimOp(exp.val) ? typeofPrim(exp.val) :
                                isClosure(exp.val) ? typeOfClosure(exp.val, _tenv, _p) :
                                    makeFailure("fail");

export const typeOfClosure = (exp : Closure, _tenv: TEnv ,_p : Program): Result<TExp> => typeofExp(exp.body[exp.body.length-1], _tenv, _p)
...
export const typeofTypeCase = (exp: TypeCaseExp, tenv: TEnv, p: Program): Result<TExp> =>
    bind(checkTypeCase(exp,p),() =>
        bind((mapResult((c: CaseExp) =>
                bind(getRecordByName(c.typeName, p), (r:Record) =>
                    typeofExp(c.body[c.body.length-1],
                        makeExtendTEnv(map((varD: VarDecl)=> varD.var, c.varDecls),map((field: Field) => field.te, r.fields),tenv), p)), exp.cases)),
            (types:TExp[]) => checkCoverType(types,p)))
